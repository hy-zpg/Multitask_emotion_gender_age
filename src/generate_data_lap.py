...
root_dir = '/home/yanhong/Downloads/next_step/Multitask_emotion_based/LAP_data_2/'

train_src = root_dir + 'Train.csv'
val_src   = root_dir + 'Validation.csv'
img_src   = root_dir + 'images/'
...
def get_train_list(cast_age_to_int=True):
    #print len(get_list(train_src, cast_age_to_int))
    return get_list(train_src, cast_age_to_int)

def get_val_list(cast_age_to_int=False):
    #print len(get_list(val_src, cast_age_to_int))
    ...
    while 1:
        ...
        for k in xrange(batch_size):
            ...
            if RANDOM_CROP:
                ...
                x_off=w_max_half
                y_off=h_max_half
            #origin_img = image.img_to_array(Image.open(img_src+dat['add']) 
            #print origin_img.shape
            img=image.img_to_array(Image.open(img_src+dat['add']).convert('RGB').resize( (w0,h0)))
            #img = origin_img.convert('RGB').resize((w0,h0))
