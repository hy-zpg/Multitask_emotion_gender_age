...
def multi_task_face():
    ...
    model=Model(inputs=model.input,outputs=drop9_out)
    plot_model(model,to_file='./task_lap/model_lower.png',show_shapes=True)
    return model
...
age_conv = Convolution2D(num_outputs_age, (1, 1), padding='valid', name='age_conv')(common_feat)
...
model=Model(inputs = image_batch, 
            outputs = age_out)
plot_model(model,to_file='./task_lap/model_upper.png',show_shapes=True)
model.summary()


list_dat_train_age = gd_lap.get_train_list()
list_dat_valid_age = gd_lap.get_val_list(cast_age_to_int=True)
gen_train_age = gd_lap.generate_data(list_dat_train_age, batch_size, h0, w0, crop_size, SHUFFLE=True, RANDOM_CROP=True)
gen_valid_age = gd_lap.generate_data(list_dat_valid_age, batch_size, h0, w0, crop_size, SHUFFLE=True, RANDOM_CROP=False)

base_lr = 0.0001
rmsprop=keras.optimizers.Adam(lr=base_lr)
model.compile(loss='categorical_crossentropy', optimizer=rmsprop, metrics=['accuracy'])
EPOCH_NB = 100
BATCH_NB = len(list_dat_train_age)/batch_size
BATCH_NB_TEST = len(list_dat_valid_age)/batch_size
...
for epoch_nb in xrange(EPOCH_NB):
    if epoch_nb>1 and lr_schedule[epoch_nb] != lr_schedule[epoch_nb-1]:
        model.save('./task_lap/model_epoch_%d_weights.h5'%epoch_nb)
    ...
    pickle.dump({'LOSSES':LOSSES, 'LOSSES_test':LOSSES_test} , open("./task_lap/historty.pickle", 'w'))
end = time.clock()
print 'total_lap_cost_time %d.' %  (end-start)
model.save('./task_lap/model_last_weights.h5')
