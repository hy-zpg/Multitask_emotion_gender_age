...
def multi_task_face():
    ...
    plot_model(model,to_file='../task_two/model_lower.png',show_shapes=True)
...
plot_model(model,to_file='../task_two/model_upper.png',show_shapes=True)
model.summary()
...
model.compile(optimizer='rmsprop',
              loss={'age_out': 'categorical_crossentropy', 'gender_out': 'binary_crossentropy','smile_out': 'binary_crossentropy','glass_out': 'binary_crossentropy'},
              metrics = ['mae','acc'])
...
for epoch_nb in xrange(EPOCH_NB):
    if epoch_nb>1 and lr_schedule[epoch_nb] != lr_schedule[epoch_nb-1]:
        model.save('./task_two/model_epoch_%d_weights.h5'%epoch_nb)
    ...
    pickle.dump({'LOSSES':LOSSES, 'LOSSES_test':LOSSES_test} , open("./task_two/historty.pickle", 'w'))
end = time.clock()
print 'total_two_task_cost_time %d.' %  (end-start)
model.save('./task_two/model_last_weights.h5')
